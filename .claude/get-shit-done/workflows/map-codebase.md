...
```bash
INIT=$(node "/root/cocoon-api-proxy/.claude/get-shit-done/bin/gsd-tools.cjs" init map-codebase)
...
```bash
node "/root/cocoon-api-proxy/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
