...
<template>
@/root/cocoon-api-proxy/.claude/get-shit-done/templates/UAT.md
</template>
...
```bash
INIT=$(node "/root/cocoon-api-proxy/.claude/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
Commit the UAT file:
```bash
node "/root/cocoon-api-proxy/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @/root/cocoon-api-proxy/.claude/get-shit-done/workflows/diagnose-issues.md
