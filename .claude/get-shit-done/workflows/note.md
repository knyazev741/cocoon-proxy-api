...
- **Global scope**: `/root/cocoon-api-proxy/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/root/cocoon-api-proxy/.claude/notes/`)
...
2. Glob `/root/cocoon-api-proxy/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/root/cocoon-api-proxy/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/root/cocoon-api-proxy/.claude/notes/` — works in any directory
